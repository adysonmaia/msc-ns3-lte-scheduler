...
EpcEnbApplication::RecvFromLteSocket (Ptr<Socket> socket)
{
  ...
  if (found) {
	  ...
	  if (itStats == m_m2mPgwStats.end()) {
		  ...
		  statsValue.lost_packets = 0;
		  statsValue.lost_delay = Time(0.0);
		  statsValue.lost_size = 0;
		  statsValue.rx_packets = 0;
		  statsValue.rx_delay = Time(0.0);
		  statsValue.rx_packets = 0;
		  statsValue.rx_size = 0;
		  if (delayExceeded) {
			  statsValue.lost_packets++;
			  statsValue.lost_delay += timeDiff;
			  statsValue.lost_size += packet->GetSize();
		  } else {
			  statsValue.rx_packets++;
			  statsValue.rx_delay += timeDiff;
			  statsValue.rx_size += packet->GetSize();
		  }
		  m_m2mPgwStats.insert(std::pair<uint16_t, M2mEpcStats_s>(rnti, statsValue));
	  } else {
		  if (delayExceeded) {
			  (*itStats).second.lost_packets++;
			  (*itStats).second.lost_delay += timeDiff;
			  (*itStats).second.lost_size += packet->GetSize();
		  } else {
			  (*itStats).second.rx_packets++;
			  (*itStats).second.rx_delay += timeDiff;
			  (*itStats).second.rx_size += packet->GetSize();
		  }
	  }
  }
...
M2mEpcStats_s
EpcEnbApplication::GetM2mEpcStats(uint16_t rnti) const
{
	M2mEpcStats_s response;
	std::map<uint16_t, M2mEpcStats_s>::const_iterator itStats = m_m2mPgwStats.find(rnti);
	if (itStats != m_m2mPgwStats.end()) {
		response = (*itStats).second;
	} else {
		response.lost_packets = 0;
		response.lost_delay = Time(0.0);
		response.lost_size = 0;
		response.rx_packets = 0;
		response.rx_delay = Time(0.0);
		response.rx_packets = 0;
		response.rx_size = 0;
	}
